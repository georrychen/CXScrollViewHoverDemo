...
- (void)setupUI {
    ...
    self.tableview_one.cellClickedCallback = ^(NSIndexPath * _Nonnull indexPath) {
        ...
        navVc.navigationItem.title = @"带导航栏的悬停效果";
        navVc.title = @"22";
...
- (void)hoverTableViewDidScroll:(HoverTableView *)currentTableView {
    ...
    if (topOffset > 0) { // 表格向上移动
         ...
         */
        if (topOffset >= headViewFrame.size.height) { // 到达悬停距离啦
            tooBarViewFrame.origin.y = self.kNavbarHeight;
            // 当其它表格已经悬停了，就不需要再改变 contentInset 了, 不然再来回切换时会有bug
          if (tmpTableView.contentOffset.y <= - (tooBarViewFrame.size.height)) {
                tmpTableView.contentInset = UIEdgeInsetsMake((tooBarViewFrame.size.height), 0, 0, 0);
                [tmpTableView setContentOffset:CGPointMake(0, -(tooBarViewFrame.size.height))];
            }
